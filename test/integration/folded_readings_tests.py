...
def test_folding_reads_the_strip_as_its_source_does(declared: Declare) -> None:
    rows = tuple(
    ...
    folded, left_out = check(strip_domain(declared), rows, new_evaluator())

    assert folded > 20 and left_out >= 0


def test_folding_reads_a_game_without_a_board_as_its_source_does(declared: Declare) -> None:
    rows = tuple(
        PositionRow(state, player, 0.0)
    ...
    folded, _ = check(line_domain(declared), rows, new_evaluator())

    assert folded > 0


def test_folding_reads_the_capture_game_as_its_source_does(declared: Declare) -> None:
    rows = tuple(
        PositionRow(state, player, 0.0)
        for state in (capture_position({2: "A", 6: "A", 4: "B", 7: "B"}, "A"), capture_domain(declared).start())
        for player in ("A", "B")
    )

    folded, _ = check(capture_domain(declared), rows, new_evaluator())

    assert folded > 0


def test_an_aggregate_over_pairs_or_without_parts_is_left_to_its_source(declared: Declare) -> None:
    evaluator, rows = new_evaluator(), (PositionRow(position({1: "X"}, "O"), "X", 0.0),)

    (pairs,) = evaluator.aggregate_columns(strip_domain(declared), rows, [("cell", "sum", True, ("{view}.cell[i] == me",), ())])
    (partless,) = evaluator.aggregate_columns(strip_domain(declared), rows, [("cell", "sum", False, (), ())])
