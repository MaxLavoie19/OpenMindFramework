...
| `service/one_ply_chooser.py` | `OnePlyChooser(solver, predictor, state_reader)`: the cheap ways to choose, for a player short of time: `best(domain, state, valuer, rng, deadline=None)`, the legal move whose outcomes the valuer values highest for the player to act, ties drawn at random, None when a move couldn't be valued in time; `random(domain, state, rng, unknown)`, a random legal move; `values(...)`, each legal move's value, which the fallback uses too |
| `service/move_planner.py` | `MovePlanner`: `plan(budget, moves, has_fallback, has_valuer)` picks how a move on a clock is chosen, the richest option whose cost fits the budget: the fallback then the search (the fallback's cost per legal move plus a search iteration per legal move), the search alone (a search iteration per legal move, so it looks at each move once), one-ply (a valuation per legal move), or a random move, which a budget of 0 gets; `observe(kind, seconds, count)` records what `iteration`, `fallback` and `valuation` cost, each option's cost being the mean of the game so far and a cost never measured counting as fitting |
