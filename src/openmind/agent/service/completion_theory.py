...
class CompletionTheory:
    ...
    reading a state that could be true and the parameter `player`, and the observation's definitions when it is source,
    says what the hypotheses are about, such as the other player's hidden choice alone, leaving the rest hidden."""

    def __init__(self, state_observer: StateObserver, rule_caller: RuleCaller, label: Rule | None = None) -> None:
    ...
    def hypotheses(self, domain: Domain, observed: State, player: str) -> tuple[tuple[Hypothesis, float], ...]:
        ...
        prepared = (
            None if self._label is None else self._rule_caller.prepare(self._label, (PLAYER,), observation.definitions)
        )
        ...
        for state, probability in self._state_observer.completions(observation, observed, player):
            if prepared is None:
                ...
                label = self._rule_caller.call(prepared, state, {PLAYER: player})  # type: ignore[assignment]
