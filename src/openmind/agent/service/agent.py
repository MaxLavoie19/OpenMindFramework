...
class Agent:
    ...
    With a deduction fallback, a position its rules have no clue about is deduced first, and a proven choice is made
    without searching. Where players act at once, it searches for the player it's given and samples its action from its
    average strategy.
    ...
    def _on_clock(
        ...
        if option != RANDOM_OPTION:
            if option == FULL_OPTION and self._fallback is not None:
                fallback_started, measured_from = source.now(), time.monotonic()
                deduced = self._fallback.result(rbs, state, self._valuation, deadline)
                if process_debugger().measurement_kept(measured_from):
