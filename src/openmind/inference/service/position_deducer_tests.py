...
def test_a_win_at_once_is_proven_within_one_ply(game: Game) -> None:
    deduction = new_deducer().deduce(game("tictactoe"), position(WIN_IN_1), DeductionBudget(1, 60.0))
...
def test_a_forced_win_is_proven_at_the_first_depth_that_reaches_its_end(game: Game) -> None:
    deduction = new_deducer().deduce(game("tictactoe"), position(WIN_IN_3), DeductionBudget(3, 60.0))
...
def test_a_position_where_every_move_loses_is_proven_lost(game: Game) -> None:
    deduction = new_deducer().deduce(game("tictactoe"), position(LOST_IN_2), DeductionBudget(2, 60.0))

    assert (deduction.player, deduction.payoffs, deduction.plies, len(deduction.line)) == ("O", (1.0, 0.0), 2, 2)


def test_a_win_further_than_the_plies_stays_unproven(game: Game) -> None:
    deduction = new_deducer().deduce(game("tictactoe"), position(WIN_IN_3), DeductionBudget(2, 60.0))

    assert (deduction.proven, deduction.action, deduction.payoffs, deduction.line, deduction.plies) == (False, None, None, (), 2)


def test_a_deduction_stops_when_its_seconds_run_out(game: Game) -> None:
    times = itertools.chain((0.0,), itertools.repeat(5.0))

    deduction = new_deducer(lambda: next(times)).deduce(game("tictactoe"), position(WIN_IN_3), DeductionBudget(3, 1.0))

    assert (deduction.proven, deduction.plies) == (False, 0)


def test_a_budget_without_plies_or_seconds_or_a_position_without_a_legal_action_raises(game: Game) -> None:
    rbs, deducer = game("tictactoe"), new_deducer()
