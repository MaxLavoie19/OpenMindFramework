...
def test_variables_are_attributes_and_offsets_read_around_an_index(declared: Declare) -> None:
    ...
    with pytest.raises(AttributeError, match="no model 'board'"):
        _ = view.board
...
def test_look_aheads_nest_and_read_the_views_they_close_over(declared: Declare) -> None:
    ...
    marks_gained = view.best("X", lambda move: len(move.cell.where("X")) - len(view.cell.where("X")))
...
def test_a_player_without_an_action_leaves_the_reading_as_it_is_here(declared: Declare) -> None:
    view = new_mechanics().view(strip_domain(declared), position({1: "X", 2: "X"}, "O", {"X": 1.0, "O": 0.0}))
...
def capture_domain(declared: Declare) -> RuleBasedSystem:
    ...
    effects = PythonRule(
        "cell = cell.moved((at,), (at + step,), None)\n"
        "if not cell.where('B' if turn == 'A' else 'A'):\n"
...
def capture_position(pieces: dict[int, str], turn: str) -> State:
    cells = tuple(pieces.get(at) for at in range(1, 8))
    return State.of(cell=Grid((7,), cells), payoff=Map.of({"A": None, "B": None}), turn=turn)
...
def test_what_if_views_edit_a_copy_of_the_grids_and_leave_the_position_as_it_is(declared: Declare) -> None:
    ...
    assert view.with_value("cell", (1, 4), "O").cell.cells == ("X", "O", None, "O")
    assert view.with_value("payoff", "X", 1.0).payoff["X"] == 1.0
    assert view.cleared((1, 1)).cell.cells == (None, "O", None, None)
    assert view.copied((1, 1), (1, 3)).cell.cells == ("X", "O", "X", None)
    assert view.alone((1, 2)).cell.cells == (None, "O", None, None)
...
def test_what_if_views_read_defenders_lone_moves_and_attacks_on_empty_cells(declared: Declare) -> None:
    ...
    defended = (
        view.with_value("cell", (3,), "B").changed("A", "cell", (3,)),
        view.with_value("cell", (5,), "A").changed("B", "cell", (5,)),
    )
...
def test_rules_read_the_view_as_here(declared: Declare) -> None:
    ...
    assert RuleRunner(StateNamespaceMapper()).value(rule, state, None, library.names(rbs, state)) == 1.0
...
def test_clearing_a_cell_needs_the_game_to_declare_its_grid_s_empty_value(declared: Declare) -> None:
    ...
    rbs = declared(
        ...
        outcomes={"place": ((1.0, PythonRule("cell = cell.placed((1, col), turn)")),)},
