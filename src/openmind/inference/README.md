...
| `service/position_view.py` | `PositionView`: a position as expressions read it, its models as rules read them |
...
- Its models (`openmind.structure`), as attributes read the way rules read them: a scalar as its value,
  `here.turn`; a grid, list or map as itself, `here.cell[2, 3]`, `here.payoff[me]`. A grid's coordinates are
...
number. A variable is a scalar, a grid's cell or a map's entry; a base, a grid or a map, is read at its indices:
coordinates for a grid, keys for a map. Lists aren't read. A number is never compared for equality with a value seen; it is read as it is, and thresholds, arithmetic
...
1. **Leaves:**
   ...
   - for every grid of names and every value, how many cells hold it
...
2. **Pattern children,** for grids: one more condition, on any base read at the
   pattern's index shifted by any offset seen between two indices of one base (the same index included): `==` or `!=`
   any name that base takes, `OUTSIDE`, or the variable another condition reads. A grid with other cells than the
...
3. **Aggregate children,** for any base, grid or map: the body grows by an operation (`+ - * / abs >= <= ==
...
- Tests: `service/mechanics_tests.py`, `service/position_view_tests.py`, `service/expression_generator_tests.py`,
  `service/expression_search_tests.py`, `service/position_deducer_tests.py`; integration:
  `test/integration/tictactoe_inference_tests.py`; in OpenMindChess, `test/integration/chess_inference_tests.py` and
  `test/integration/chess_deduction_tests.py`.
