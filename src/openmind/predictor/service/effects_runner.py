...
class EffectsRunner:
    """Runs effects rules: the outcome probability distribution of an action in a state, each effects rule running on
    the state with the action's parameters and, for a script, the definitions its effects see, and carrying the chance
    that outcome happens. Actions taken at once run one after another, each seeing its player as `player`, then the
    rules for what their actions together lead to run. The rule predictor runs a ruleset's effects through it."""
