...
Solver(Interpreter(VariableNameMapper())).solve(problem, state)
...
- `DEBUG Accepted <action>`
- `DEBUG Rejected <action>: constraint is false: <constraint>`
- `INFO <legal> of <candidates> candidate actions are legal`
