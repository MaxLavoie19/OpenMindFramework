...
class SearchResult:
    """The player acting at the root, every root action's statistics, the action chosen (the most visited, or where
    players act at once one sampled from the average strategy), every tree sample, where players act at once the
    searching player's average strategy over its root actions, and the iterations the search completed in the seconds
    it took, and on a clock the budget the step was given. `depth` is the most actions from the root its tree reached, and
