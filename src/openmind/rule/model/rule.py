...
class HiddenRule(Protocol):
    """The names of the state's variables the player can't see."""

    def __call__(self, state: State, player: str) -> Iterable[str]: ...


class CompletionsRule(Protocol):
    """What the hidden variables could hold, given what the player sees, with each possibility's probability."""

    def __call__(self, state: State, player: str) -> Iterable[tuple[Mapping[str, Value], float]]: ...


#: A domain's rule: Python source OMF compiles, or a function the project wrote and gives OMF, of one of the shapes
