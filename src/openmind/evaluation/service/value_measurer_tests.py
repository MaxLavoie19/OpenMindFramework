...
from openmind.evaluation.service.exact_search_tests import new_exact_search, trust
...
def new_measurer() -> ValueMeasurer:
    return ValueMeasurer(
        create_solver(), create_predictor(), StateReader(), ChoiceMeasurer(StateTextMapper(), ActionTextMapper())
    )


def test_a_position_gives_the_value_error_and_the_choice_one_step_ahead() -> None:
    domain = trust()
    values = new_exact_search().action_values(domain, domain.initial_state)

    ((error, optimal, regret),) = new_measurer().measure(domain, Fixed((0.9, 0.1)), [(domain.initial_state, values)], 0.0)
...
def test_an_unvalued_outcome_gets_the_mean_of_the_other_actions_values() -> None:
    domain = trust()
    values = new_exact_search().action_values(domain, domain.initial_state)

    ((error, optimal, regret),) = new_measurer().measure(domain, Fixed(None), [(domain.initial_state, values)], 0.0)
