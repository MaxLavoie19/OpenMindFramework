...
def first_mover_decides(declared) -> RuleBasedSystem:
...
def test_evaluate_reports_baselines_and_agreement(declared: Declare) -> None:
    settings = EvaluationSettings(games=2, iterations=5, positions=10, budgets=(5, 20), seed=1)

    report = create_evaluator().evaluate(
        first_mover_decides(declared), AgentBuilder().with_exploration(math.sqrt(2)), settings, rules_file="rules.json"
    )
...
def test_a_rater_is_compared_with_an_unguided_agent_on_the_same_positions_and_measured_alone(declared: Declare, 
    ...
    report = create_evaluator().evaluate(first_mover_decides(declared), builder, settings, "rules.json", FavorsWin())
...
def test_a_rater_rating_every_action_alike_separates_nothing_and_picks_uniformly(declared: Declare) -> None:
    ...
    report = create_evaluator().evaluate(first_mover_decides(declared), builder, settings, "rules.json", RatesAlike())
...
def test_a_valuer_is_compared_with_an_unguided_agent_on_the_same_positions_and_measured_alone(declared: Declare, 
    caplog: pytest.LogCaptureFixture,
) -> None:
    caplog.set_level(logging.INFO, logger="openmind.evaluation")
    ...
    report = create_evaluator().evaluate(
        first_mover_decides(declared), builder, settings, values_file="values.json", valuer=ValuesEven()
    )
...
def test_all_positions_takes_every_position(declared: Declare, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO, logger="openmind.evaluation")
    settings = EvaluationSettings(games=0, iterations=5, positions=None, budgets=(5,), seed=1)

    report = create_evaluator().evaluate(first_mover_decides(declared), AgentBuilder().with_exploration(math.sqrt(2)), settings)

    assert [(item.iterations, item.positions) for item in report.agreement] == [(5, 1)]
    assert "Every action is optimal in 0 of 1 positions" in caplog.messages


def test_a_reference_search_stands_in_for_perfect_play_on_positions_of_random_games(declared: Declare, 
    caplog: pytest.LogCaptureFixture,
) -> None:
    caplog.set_level(logging.INFO, logger="openmind.evaluation")
...
def test_a_reference_search_needs_a_number_of_positions(declared: Declare) -> None:
    ...
    with pytest.raises(ValueError, match="number of positions"):
        create_evaluator().evaluate(first_mover_decides(declared), AgentBuilder().with_exploration(math.sqrt(2)), settings)


def test_no_positions_skips_agreement_and_the_exact_search(declared: Declare, caplog: pytest.LogCaptureFixture) -> None:
    caplog.set_level(logging.INFO, logger="openmind.evaluation")
    settings = EvaluationSettings(games=2, iterations=5, positions=0, budgets=(5, 20), seed=1)

    report = create_evaluator().evaluate(
        first_mover_decides(declared), AgentBuilder().with_exploration(math.sqrt(2)), settings, "rules.json", FavorsWin()
    )
