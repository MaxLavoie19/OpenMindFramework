...
class ArmSelector:
    """Chooses which two heuristics play a game against each other, by UCB1 on their game scores.

    Ranking heuristics is what a bandit is for: a heuristic that looked poor in its first game may have been unlucky,
    and one that looked good may not be, so how much to keep trying each is exactly the question UCB1 answers. A
    heuristic playing itself says nothing — it wins half its games by symmetry — so every game is between two of them.
