...
class SelfPlay:
    ...
    def play_arms(
        ...
        """Every game between two arms, in order. `scores` gives each arm's games and points before these games, which
        ...
        def arguments_for(index: int) -> tuple[object, ...]:
            ...
            for arm in seated:
                pending[arm] += 1
            return domain, tuple(builders[arm] for arm in seated), seated, agent_seeds[index], outcome_seeds[index], keep_samples
    ...
    def play_game(
        ...
        game, sampled, last = self._game(domain, (agent,) * len(domain.players.names), outcome_seed, keep_samples)
        logger.info(
            "Self-play game with seeds %d and %d finished in %d plies%s: %d samples, payoffs %s",
            agent_seed,
            outcome_seed,
            len(game.states),
            self._ending(domain, last),
            sampled,
            " ".join(f"{name}={payoff}" for name, payoff in zip(domain.players.names, game.payoffs)),
        )
    ...
    def play_arm_game(
        ...
        game, sampled, last = self._game(domain, agents, outcome_seed, keep_samples)
        logger.info(
        ...
        self._log_record(domain, game, f"Self-play game with seeds {agent_seed} and {outcome_seed}")
        return PlayedGame(game.samples, game.states, game.search_values, game.payoffs, arms, game.actions)

    def _game(
        self, domain: Domain, agents: Sequence[Agent], outcome_seed: int, keep_samples: bool
        ...
        while self._solver.solve(domain.problem, state):
            agent = agents[self._state_reader.player_to_act(state, domain.players)]
        ...
        game = PlayedGame(tuple(samples), tuple(states), tuple(search_values), payoffs, (), tuple(actions))
        return game, sampled, state

    def _ending(self, domain: Domain, state: State) -> str:
        """` by <why the game ended>` when the domain says, nothing otherwise."""
