...
| `service/self_play.py` | `SelfPlay`: an agent plays a domain against itself, games in the task runner's workers; returns every game, with its searches' samples unless `keep_samples` is false, and leaves out a game that took its worker over the memory cap in a fresh worker too; `play_arms(domain, builders, games, scores, selector, exploration, rng)` plays games between arms, a game's two arms chosen when a worker starts it |
| `model/played_game.py` | `PlayedGame(samples, states, search_values, payoffs, arms=(), actions=())`: a self-play game's search samples, its positions with the search's mean payoff for the player to act in each, its final payoffs, in a game between arms the arm each player followed, and the actions played in order |
...
| `service/value_training_loop.py` | `ValueTrainingLoop`: rounds of self-play valuing positions with the previous round's rules, value fitting, and games against the random policy, untrained MCTS and the previous round's agent; with the signals target and no value bases yet, it prepares the deduced signals first |
...
- `openmind.training.service.self_play`: `INFO Self-play game with seeds <agent seed> and <outcome seed> finished in
  ...
  ` by <ending>` after the plies when the domain says why games end (see `agent/README.md`); and `INFO Self-play game
