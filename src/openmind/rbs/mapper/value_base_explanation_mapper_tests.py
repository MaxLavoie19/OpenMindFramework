...
def test_explained_rules_become_a_table_with_a_header_row() -> None:
    base = ValueBase("chess", -0.2, 0.0, 1.0, (ValueRule(PythonRule("(here.x[me]) | 1"), 0.5),))
    explanations = (
    ...
    assert ValueBaseExplanationMapper().to_markdown(base, explanations) == (
        "# chess value rules\n"
        "\n"
        "2 rules; bias -0.2; payoffs from 0 to 1; explained by qwen3:8b.\n"
        "\n"
