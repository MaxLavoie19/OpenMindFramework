...
| `service/term_evaluator.py` | `TermEvaluator`: a term's values on position rows as numbers; several terms at once in the task runner's workers, the rows split in slices; `aggregate_columns(domain, rows, parts)` folds an aggregate from its readings instead of running its source, `parts` being `(base, kind, pair, readings, operations)`, and gives None for what a reading kept per cell can't answer: an aggregate over pairs of indices, or one whose body didn't record its parts |
| `service/reading_cache.py` | `ReadingCache`: what a reading gives at every index of a base, kept per position, player and reading, so every candidate sharing a reading reads it once; cleared with the views, by `limit_memory` and by the process's memory guard |
