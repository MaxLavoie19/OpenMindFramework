...
def test_build_gives_a_rater_using_the_rule_base() -> None:
    rater = RuleRaterBuilder().with_rule_base(RuleBase("test", (Rule("press", (), 0.5, 40),))).with_domain(LIGHTS).build()
...
def test_build_rejects_a_missing_rule_base() -> None:
    with pytest.raises(ValueError, match="rule base"):
        RuleRaterBuilder().with_domain(LIGHTS).build()


def test_build_rejects_a_missing_domain() -> None:
    with pytest.raises(ValueError, match="domain"):
        RuleRaterBuilder().with_rule_base(RuleBase("test", ())).build()
